...
//API
import { listProducts } from '@api/request/products';
...
export default function ProductsScreen({ navigation, }) {
    ...
    const [type, settype] = useState('Em andamento');
    const types = ['Em andamento', 'Concluídos', 'Cancelados',];
                ...
                <TopMenu back={false} search={false} />
                    ...
                    <Column>
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ columnGap: 12 }}>
                            <Button onPress={() => { navigation.goBack()}} ph={1} pv={1} style={{ justifyContent: 'center', alignItems: 'center', width: 40, height: 40, marginLeft: 28, }} bg="#fff">
                                <ArrowLeft size={20} color={color.label} />
                        </Button>
                        
                        {types.map((item, index) => ( 
                            <Button style={{ backgroundColor: type === item ? color.sc.sc3 : color.sc.sc3+30, }} ph={15} pv={10} onPress={() => {settype(item)}} >
                                <LabelBT size={14} style={{ textAlign: 'center', color:  type === item ? '#fff' : color.sc.sc3, }}>{item}</LabelBT>
                            </Button>
                        ))}
