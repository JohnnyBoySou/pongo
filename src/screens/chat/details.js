...
export default function ChatDetailsScreen({ navigation, route }) {
    ...
    return (
        <Main>
            ...
            <Modal ref={modalCamera} snapPoints={[0.1, SCREEN_HEIGHT - 70]} onClose={() => setopenCamera(false)} style={{ zIndex: 99, }}>
                {openCamera && <CameraChat token={token} user={user} setopenCamera={setopenCamera} modalCamera={modalCamera} />}
            </Modal>
        </Main>
    )
}
