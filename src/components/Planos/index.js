...
const CardPlano = ({ item, destino }) => {
    ...
    return (
        <Column style={{ width: 250, borderWidth: 1, borderColor: id % 2 != 0 ? 'transparent' : color.border, backgroundColor: id % 2 === 0 ? 'transparent' : '#F7F7F7', borderRadius: 18, paddingHorizontal: 20, }}>
            <Row style={{ justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 20, marginBottom: 8, }}>
                <Title size={18}>Plano {name}</Title>
                <Label size={16}>{price}</Label>
            </Row>
            <Label>{date}</Label>
